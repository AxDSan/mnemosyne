...
class TestPolyphonicVecVoiceBoundary:
    """Unmarked stores scan vec rows without raw-L2 KNN, while JSON-only
    episodic rows remain available through the numpy fallback."""
    ...
    def test_unmarked_store_abstains_from_knn(self, beam_db, monkeypatch):
        ...
        # JSON-only rows must not overlap the unrelated vec rowids above;
        # they remain available alongside the representation-safe scan.
