...
| `fts_weight` | `MNEMOSYNE_FTS_WEIGHT` | `0.3` [^fts_weight] | no | FTS5 rank weight in hybrid ranking. |
...
| `importance_weight` | `MNEMOSYNE_IMPORTANCE_WEIGHT` | `0.2` [^importance_weight] | no | Importance weight in hybrid ranking. |
...
| `llm_file` | `MNEMOSYNE_LLM_FILE` | `MiniCPM5-1B-Q4_K_M.gguf` [^llm_file] (declared *(unset)*) | **yes** | GGUF filename within the repo. |
...
| `llm_repo` | `MNEMOSYNE_LLM_REPO` | `openbmb/MiniCPM5-1B-GGUF` [^llm_repo] (declared *(unset)*) | **yes** | Hugging Face repo holding the local GGUF model. |
...
| `vec_weight` | `MNEMOSYNE_VEC_WEIGHT` | `0.5` [^vec_weight] | no | Vector similarity weight in hybrid ranking. Normalized with the next two to sum to 1.0. |
...
For these keys a module-level constant reads the environment variable directly, so the value in `DEFAULTS` is not what the runtime uses. Treat the environment variable as authoritative.

[^fts_weight]: `fts_weight` -- Read directly from the environment by `_normalize_weights`.
[^host_llm_n_ctx]: `host_llm_n_ctx` -- `local_llm.py` reads `MNEMOSYNE_HOST_LLM_N_CTX` directly with a `32000` fallback, so the `2048` declared in `config.py` never applies.
[^importance_weight]: `importance_weight` -- Read directly from the environment by `_normalize_weights`.
[^llm_file]: `llm_file` -- `local_llm.py` falls back to `DEFAULT_MODEL_FILE` when the env var is unset.
[^llm_repo]: `llm_repo` -- `local_llm.py` falls back to `DEFAULT_MODEL_REPO` when the env var is unset.
[^vec_weight]: `vec_weight` -- `_normalize_weights` in `beam.py` reads the env var directly, so `config.yaml` alone does not affect recall.
