...
- **Auto-sleep is scoped to the triggering session, and `auto_sleep_enabled: false` is honored (#771).** The Hermes provider's `_maybe_auto_sleep()` selected the sleep path by capability probing (`hasattr(beam_ref, "sleep_all_sessions")`), which is always true for `BeamMemory`; auto-sleep therefore ran `sleep_all_sessions()`, sweeping every session in the database. On a replica of a shared-surface DB the rows of the MCP's fixed `mcp_{bank}` session arrive aged but unconsolidated, so one write collapsed weeks of backlog into a gist (407 -> 147 working rows). The worker now always runs the session-scoped `sleep()` on a beam bound to the triggering `session_id` (restoring the #342 contract). The provider also reads the core `auto_sleep_enabled` config key (via the Mnemosyne config bridge, matching the root provider) in addition to the Hermes `auto_sleep` key, so `mnemosyne config set auto_sleep_enabled false` actually disables auto-sleep.
