...
- **MCP invalidate no longer records a supersession link that leads nowhere (#676).** `mnemosyne_invalidate` resolves `replacement_id` against the working and episodic tiers under the same scope rule it applies to the target row. An unknown or out-of-scope replacement returns `replacement_not_found` and leaves the memory untouched, instead of reporting `invalidated` while writing a dangling `superseded_by` that hides the row from recall with nothing to follow.
