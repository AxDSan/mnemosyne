...
### Fixed
- **`_parse_facts()` no longer returns one copy of a statement per category it appears in.** The extraction prompt defines `facts`, `instructions`, `preferences` and `timelines` as overlapping sets — "always use tabs" is both a fact and an instruction, and a dated statement is both a fact and a timeline — so a model that files one statement under every category it belongs to is answering correctly. `_parse_facts()` flattened those categories with `extend()` and no dedupe, so that correctness was punished: the statement came back once per category and was stored once per copy, into a `facts` table keyed only on `fact_id` and therefore with no constraint to absorb the repeats. Observed with Qwen3.5-9B, which populates `instructions` and `timelines` faithfully and so emitted 6 items for 4 distinct statements; models that leave those categories empty were never affected, which is why this went unnoticed. Items are now deduped on trimmed, case-folded text, first occurrence winning, so `facts` ordering is preserved. Distinct statements are unaffected.
