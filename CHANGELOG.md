...
- **The OpenAI-compatible modality test stub now serves scripted responses deterministically (#798).** Its localhost server handles one request at a time and records response statuses, so client-error retry assertions cannot consume a response out of order.
