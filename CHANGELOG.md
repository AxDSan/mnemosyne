...
- **MCP tool schemas advertise the optional `bank` parameter (#769).** Every MCP-callable tool whose handler resolves a memory bank now exposes `bank` in its schema, with the resolution precedence documented (explicit `bank` argument → `MNEMOSYNE_MCP_BANK` env var → `default`). The shared `BANK_PROPERTY` keeps the 23 bank-aware schemas consistent; `mnemosyne_validate` is intentionally excluded because its `bank` means `private|surface`, as are the shared-surface tools (`mnemosyne_shared_*`), which resolve no bank. A schema-driven parity test derives bank-awareness from the handler registry (`_resolve_bank` usage), so a handler that starts or stops resolving banks without updating its schema fails CI. The generated `docs/api/tool-schema.mdx` is regenerated, and the docs generator now resolves shared constant references inside schema dicts instead of dropping them.
