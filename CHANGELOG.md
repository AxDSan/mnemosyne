...
- **Recall results now include persisted metadata (#665).** Working and episodic results expose parsed `metadata` dictionaries through both linear and polyphonic recall. Metadata is hydrated in at most one query per tier after ranking, avoiding per-result consumer lookups without changing scores or ordering. Missing, malformed, and non-object metadata degrades to `{}`.
