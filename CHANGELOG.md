...
- **Concurrent first-open schema migrations are idempotent (#663).** `_add_column_if_missing` now re-reads the schema after a racing `ALTER TABLE` failure and suppresses the error only when another initializer demonstrably added the requested column. Unrelated DDL failures still propagate.
