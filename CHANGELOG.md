...
- **CLI failures now surface static machine-readable error codes (#612).** Every CLI error path — the top-level `run_cli()` dispatch boundary, `doctor`/`repair` report build & write, `diagnose`, `backup`/`restore`/`verify`, `migrate`, and the `hygiene` audit/status/clean/restore preflight — exits 1 with a stable `Error: <snake_case_code>` line instead of leaking exception text, absolute paths, or library internals to stderr. Boundary-rejected `backup` output paths keep their caller-visible message and exit code 2 (the pre-existing argument-validation contract). Unexpected exceptions escaping any handler are caught at the `run_cli()` boundary and reported as `Error: cli_unexpected_failure` with exit 1, never a traceback.
