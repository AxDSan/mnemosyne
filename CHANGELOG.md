...
- **The local consolidation model downloaded silently (#703).** The first uncached use fetched the ~656 MB `MiniCPM5-1B-Q4_K_M.gguf` from Hugging Face with no output, blocking the caller for the length of the transfer. It is reached from `sleep()` and also from `llm_available()`, which callers do not expect to download anything, so it presented as a hang; #686 documented the download without announcing it at the point it happens. `_download_model()` now logs one warning immediately before an uncached fetch, naming the model, repo, approximate size, destination directory and the `MNEMOSYNE_LLM_ENABLED=false` AAAK-only opt-out. It is a logger warning rather than a print, so it reaches stderr without disturbing the stdout the CLI and MCP server own. A cached model stays silent, an overridden model does not claim the built-in size, and a failed download still falls back to AAAK encoding as before.
