...
- **Recall no longer silently misses leading-hyphen query fragments (#744).** Queries containing leading-hyphen fragments such as ``rm -rf`` or ``--force`` could produce invalid FTS5 queries or no usable FTS terms (a token must start with a word character, and FTS5 treats a leading ``-`` as the NOT / column-exclusion operator), so recall returned an empty list without an error. Leading-hyphen fragments are now split into their components and matched through the FTS5 and lexical paths; ``-v``-style single-character flags are included while stopwords and digits stay excluded.
