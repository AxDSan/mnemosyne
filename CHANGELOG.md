...
- **Embedding API failures are no longer silent (#664).** Terminal HTTP failures (including a wrong OpenAI-compatible base path), exhausted network retries, and unexpected transport errors now emit actionable warnings before dense generation degrades to the existing `None` path. Warnings include the status/endpoint or exception type without logging request content or credentials.
