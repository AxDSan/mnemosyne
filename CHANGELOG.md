...
- **OrcaRouter LLM provider preset (#732).** `MNEMOSYNE_LLM_PROVIDER=orcarouter` resolves the OpenAI-compatible base URL `https://api.orcarouter.ai/v1` and the default model `orcarouter/auto` (a virtual router that picks the best upstream per request). Model ids are namespaced (`openai/...`, `anthropic/...`); a concrete model can be pinned with `MNEMOSYNE_LLM_MODEL=openai/gpt-4o-mini`. Context window and pricing vary by upstream, so the preset records them as unknown rather than stating fixed figures. Existing `MNEMOSYNE_LLM_BASE_URL` / `MNEMOSYNE_LLM_MODEL` overrides still win over the preset.
